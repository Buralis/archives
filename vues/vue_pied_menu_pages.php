    ...
    <ul class="pagination justify-content-center">
        ...
        if($total/$_GET['nb']<8) {
            for ($i = 1; $i <= float_page($total/$_GET['nb']); $i++) {
                if ($i == $_GET['page']) {?>
                    <li class="page-item active">
                        <a class="page-link active" onclick="includeGET('page=<?php echo $_GET['page']; ?>')"><?php echo $_GET['page']; ?>
                        <span class="sr-only">(current)</span></a>
                    </li><?php
                } else {?>
                    <li class="page-item">
                        <a class="page-link" onclick="includeGET('page=<?php echo $i; ?>')"><?php echo $i; ?></a>
                    </li><?php
                }
            }
        }
            ...
            for ($i =  float_page($total/$_GET['nb']-7); $i<= float_page($total/$_GET['nb']); $i++){
                if($i == $_GET['page']){?>
                    <li class="page-item active" >
                        <a class="page-link active" onclick="includeGET('page=<?php echo  $_GET['page']; ?>')"><?php echo $_GET['page']; ?> <span class="sr-only">(current)</span></a>
                    </li><?php
                }else{?>
                    <li class="page-item" >
                        <a class="page-link" onclick="includeGET('page=<?php echo  $i; ?>')"><?php echo $i; ?></a>
